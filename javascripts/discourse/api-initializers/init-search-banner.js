  ...
  api.decorateWidget("create-topic:after", helper => {
    if (Discourse.User.current()) {
      ...
      return helper.h("button", {
        className: "hover:bg-[#F9D973] bg-yellow text-white font-bold py-2 px-4 rounded-full",
        onclick: createTopic
      }, 'Frage stellen' );
    } else {
      return helper.attach("button", {
        label: "log_in",
        className: "hover:bg-[#F9D973] bg-[#F9D973] text-white font-bold py-2 px-4 rounded-full",
        action: "showLogin",
        icon: "user",
